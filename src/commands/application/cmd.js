...
export default function setup() {
    ...
    application
        ...
        .helpOption("-h, --help", "Help")
        .addOption(common.deploymentOption)
        .addOption(new Option("-i, --id <id>", "Id of application. If specified, -a and -A are ignored."))
        .addOption(new Option("-f, --file <file>", "Name of the file to write the exported application(s) to. Ignored with -A."))
        .addOption(new Option("-a, --all", "Export all applications in a realm. Ignored with -t."))
        .addOption(new Option("-A, --allSeparate", "Export all applications in a realm as separate files <id>.json. Ignored with -s or -a."))
            ...
            if(await getTokens()) {
                ...
                fs.readFile(command.opts().file, 'utf8', function (err, data) {
                    ...
                    if(validateImport(applicationData.meta)) {
                        for(let id in applicationData.application) {
                            if(storage.session.getItem("tenant") != applicationData.meta.origin) {
                                checkTargetCompatibility("oauth2Provider", applicationData.application[id]._provider, targetOauthServiceData);                            
                            }
